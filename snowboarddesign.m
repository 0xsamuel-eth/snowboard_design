...
%d_m = 7; %Carbon AS-4 in um
%d_m2 = 10; %S-glass in um
%vol_m = 0; %total volume of matrix in material
%vol_r = 0; %total volume of reinforcement/fiber in material
%vol_tot = vol_m + vol_r; %total volume of material

%V_f = vol_r / vol_tot; %fiber volume fraction
%V_m = vol_m / vol_tot; %matrix volume fraction

V_f = 0.6;
V_m = 0.4;

vol_tot = 4*(.20^2); %example laminate volume (4r^2)
vol_f = vol_tot *V_f;
vol_m = vol_tot *V_m;

E_f = 235e9; %Carbon AS-4 in GPa
G_f = 20e9; %Carbon AS-4 in GPa
E_m = 4.3e9; %Epoxy 3501-6 in GPa
G_m = 1.6e9; %Epoxy 3501-6 in GPa

%E1, E2, v12, G12

E1 = E_f*V_f + E_m*V_m;
E2 = (E_f*E_m) / ( E_f*V_m + E_m*V_f );
v12 = vol_m*V_m + vol_f*V_f;
G12 = (G_f*G_m) / ( V_m*G_f + V_f*G_m );
...
t = 0.001; %m, thickness of one ply
h = 0.004; %m, height
theta = [90 0 0 90];
...
z_new = [z(1) z(2) z(2) z(3) z(3) z(4) z(4) z(5)];
